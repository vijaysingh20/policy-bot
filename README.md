...
Architecturally equivalent to the class reference build â€” embeddings â†’
FAISS retrieval â†’ LCEL RAG chain â†’ cross-encoder re-ranking â†’ RAGAS
evaluation â€” applied to a different domain and document type (HR Policy
...
| Observability | LangSmith (optional â€” tracing is a no-op if `LANGSMITH_TRACING` is unset) |
...
local development. The app is upload-driven â€” no PDF ships with the repo.
...
re-rank the *same* pool down to 3 â€” isolating the re-ranker's effect from
...
passage together, a near-perfect topical match â€” despite containing no
...
(`is_table_of_contents` in `backend/ingestion/pdf_loader.py`) â€” a page
...
| `BAAI/bge-reranker-base` (12Ã— larger) | 1.00 | 0.89 |

**Decision rule, set before running the comparison:** switch models only if
one beats the FAISS baseline by â‰¥0.05 MRR. Neither does â€” `bge-reranker-base`
is better than the smaller model but still loses to no re-ranking at all â€”
...
in one chapter â€” the condition under which re-ranking should matter most.
...
| paraphrase | 0.79 | 0.69 (âˆ’0.10) |
| multi_part | 1.00 | 0.90 (âˆ’0.10) |
| **all** | **0.80** | 0.81 (+0.01) |
...
questions FAISS missed entirely (e.g. resignation notice: miss â†’ rank 1),
...
worse (0.69 MRR) â€” the larger model that won on Bennett is not universally
better.
...
inventing a percentage â€” the desired behavior. The near-zero scores are a
...
that nothing retrieved was useful â€” because for this question, nothing in
...
completed evaluations â€” variation that reflects genuine differences in
...
- **Unit** â€” schema validation, citation sanitization, chunking, the
  evaluation state machine. Fast, deterministic, no models loaded.
- **Fake-model** â€” retrieval and re-ranking logic tested against a real
  ...
  (`tests/unit/test_query.py`) â€” including a regression test for a bug
...
- **API** â€” FastAPI's `TestClient` against the real app with models
...
- **Storage is ephemeral.** Uploaded PDFs, the SQLite evaluation state, and
  the JSONL log do not persist across a Space restart â€” acceptable for a
